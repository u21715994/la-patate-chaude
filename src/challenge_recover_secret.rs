...
pub fn recover_secret(input: RecoverSecretInput) -> RecoverSecretOutput {
    ...
    while x <= secret.len() || x == 1 {
        ...
        for i in 0..input.tuple_sizes.len() {
            ...
            for j in k..(k + input.tuple_sizes.get(i).unwrap()) {
                if (secret.len() != 0)
                    && (input.letters.chars().nth(j).unwrap()
                        == *secret.get(secret.len() - x).unwrap())
                {
        ...
        for i in 0..input.tuple_sizes.len() {
            for j in k..(k + input.tuple_sizes.get(i).unwrap()) {
                if temp.contains(&input.letters.chars().nth(j).unwrap()) {
                    if (j != 0)
                        && ((j - 1) >= k)
                        && (!secret.contains(&input.letters.chars().nth(j - 1).unwrap()))
                    {
                        while temp.contains(&input.letters.chars().nth(j).unwrap()) {
                            let index = temp
                                .iter()
                                .position(|x| *x == input.letters.chars().nth(j).unwrap())
                                .unwrap();
                            temp.remove(index);
                        }
                    }
    ...
    let output = RecoverSecretOutput { secret_sentence: s };
    return output;
}
