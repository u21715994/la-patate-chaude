...
pub fn monstrous_maze(input: MonstrousMazeInput) -> MonstrousMazeOutput {
    ...
    let mut grid2: String = input.grid.clone();
    ...
    while input.grid.chars().nth(p).unwrap() != 'X' {
        if (p == y)
            && (!list_first_directions.contains(&'v'))
            && ((y + c + 1) < input.grid.len())
            && ((input.grid.chars().nth(y + c + 1).unwrap() == ' ')
                || (input.grid.chars().nth(y + c + 1).unwrap() == 'X')
                || ((input.grid.chars().nth(y + c + 1).unwrap() == 'M') && (endurance2 > 1)))
        {
            ...
            && (!list_first_directions.contains(&'>'))
            && ((y + 1) < input.grid.len())
            && ((input.grid.chars().nth(y + 1).unwrap() == ' ')
                || (input.grid.chars().nth(y + 1).unwrap() == 'X')
                || ((input.grid.chars().nth(y + 1).unwrap() == 'M') && (endurance2 > 1)))
        {
        ...
        if input.grid.chars().nth(p).unwrap() == 'X' {
            let output = MonstrousMazeOutput { path: path };
            return output;
        }
        if path.len() == 0 {
            let output = MonstrousMazeOutput {
        ...
        while (input.grid.chars().nth(p).unwrap() == ' ')
            || (input.grid.chars().nth(p).unwrap() == 'M')
        {
            grid2.replace_range(
                grid2
                    .char_indices()
        ...
        while path.len() >= 1 {
            if (path.chars().nth(path.len() - 1).unwrap() == 'v')
                && (last_removed == 'v')
                && (!list.contains(&(p - 1)))
                && ((input.grid.chars().nth(p - 1).unwrap() == ' ')
                    || (input.grid.chars().nth(p - 1).unwrap() == 'X')
                    || ((input.grid.chars().nth(p - 1).unwrap() == 'M') && (endurance2 > 1)))
            {
                ...
                path = path + "<";
                break;
            } else if (path.chars().nth(path.len() - 1).unwrap() == 'v')
                && (last_removed == 'v')
                && (!list.contains(&(p + 1)))
                && ((input.grid.chars().nth(p + 1).unwrap() == ' ')
                    || (input.grid.chars().nth(p + 1).unwrap() == 'X')
                    || ((input.grid.chars().nth(p + 1).unwrap() == 'M') && (endurance2 > 1)))
            {
                ...
                && (last_removed == 'v')
                && (!list.contains(&(p - c - 1)))
                && ((input.grid.chars().nth(p - c - 1).unwrap() == ' ')
                    || (input.grid.chars().nth(p - c - 1).unwrap() == 'X')
                    || ((input.grid.chars().nth(p - c - 1).unwrap() == 'M') && (endurance2 > 1)))
            {
